...
curl -X GET -v \
  <code>{sanitizeChromaServerUrl($chromaServerUrlStore) || 'https://chromadb.example.com'}</code>/api/v1/collections/<code>{$inputCollectionIdStore || '{COLLECTION_ID}'}</code>/count \
